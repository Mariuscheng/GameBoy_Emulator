...
pub struct MMU {
    ...
    pub mbc5: crate::core::mmu::mbc5::MBC5, // MBC5 控制器
}

impl MMU {
    pub fn new() -> Self {
        log_to_file("MMU::new() - Initialization started");
        ...
        let mut mmu = Self {
            ...
            lyc: 0,
            instance_id,
            mbc5: crate::core::mmu::mbc5::MBC5::new(),
        };

        // Initialize basic graphics data into VRAM
        mmu.init_default_graphics();
        log_to_file("MMU::new() - Initialization completed");
        mmu
    }
    ...
    pub fn read_byte(&self, address: u16) -> Result<u8> {
        let value = match address {
            ...
            0x4000..=0x7FFF => {
                // Bank N (MBC5)
                self.mbc5.read_rom(address, &self.cartridge_rom)
            }
            0x8000..=0x9FFF => self.video_ram[address as usize - 0x8000],
            0xA000..=0xBFFF => {
                // External RAM (MBC5)
                self.mbc5.read_ram(address)
            }
    ...
    pub fn write_byte(&mut self, address: u16, value: u8) -> Result<()> {
        match address {
            // ROM area (MBC5 control)
            0x0000..=0x7FFF => {
                self.mbc5.write_rom(address, value);
                Ok(())
            }
            0x8000..=0x9FFF => {
                // Check LCD status
                ...
                if mode == 3 {
                    return Err(Error::Hardware(HardwareError::MemoryWrite(address)));
                }
                if address as usize - 0x8000 < self.video_ram.len() {
                    ...
                    if can_write {
                        ...
                        log_to_file(&format!(
                            "[MMU Debug] VRAM Write: [0x{:04X}] = 0x{:02X}",
                            address, value
                        ));
                        if address as usize - 0x8000 < 8 {
                            log_to_file(&format!(
                                "[ROM→VRAM] 寫入 VRAM: addr={:04X}, value={:02X}",
                                address, value
                            ));
                        }
                        if let Ok(mut file) = std::fs::OpenOptions::new()
            ...
            0xA000..=0xBFFF => {
                self.mbc5.write_ram(address, value);
                Ok(())
            }
            0xC000..=0xDFFF => {
