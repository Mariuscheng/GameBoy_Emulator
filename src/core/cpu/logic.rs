...
pub fn dispatch(cpu: &mut CPU, opcode: u8) -> crate::core::error::Result<CyclesType> {
    match opcode {
        ...
        0xA0..=0xA7 => {
            ...
            RegTarget::from_bits(src).and_then(|target| Ok(cpu.and_a_r(target)))
        }
        // AND 立即數 (E6)
        0xE6 => Ok(cpu.and_a_r(RegTarget::Immediate)),
        // OR 指令族 (B0~B7)
        0xB0..=0xB7 => {
            let src = (opcode & 0x07) as u8;
            RegTarget::from_bits(src).and_then(|target| Ok(cpu.or_a_r(target)))
        }
        // OR 立即數 (F6)
        0xF6 => Ok(cpu.or_a_r(RegTarget::Immediate)),
        // XOR 指令族 (A8~AF)
        0xA8..=0xAF => {
            let src = (opcode & 0x07) as u8;
            RegTarget::from_bits(src).and_then(|target| Ok(cpu.xor_a_r(target)))
        }
        // XOR 立即數 (EE)
        0xEE => Ok(cpu.xor_a_r(RegTarget::Immediate)),
        // CP 指令族 (B8~BF)
        0xB8..=0xBF => {
            let src = (opcode & 0x07) as u8;
            RegTarget::from_bits(src).and_then(|target| Ok(cpu.cp_a_r(target)))
        }
        // CP 立即數 (FE)
        0xFE => Ok(cpu.cp_a_r(RegTarget::Immediate)),
        // CPL 指令 (2F)
        0x2F => Ok(cpu.cpl()),
        // SCF 指令 (37)
        0x37 => Ok(cpu.scf()),
