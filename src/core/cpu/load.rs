...
use crate::core::error::{Error, InstructionError, RegTarget, Result};
use std::io::Write;
...
pub fn dispatch(cpu: &mut CPU, opcode: u8) -> Result<CyclesType> {
    match opcode {
        ...
        // LD r, n
        0x06 => cpu.ld_r_n(RegTarget::B),
        0x0E => cpu.ld_r_n(RegTarget::C),
        0x16 => cpu.ld_r_n(RegTarget::D),
        0x1E => cpu.ld_r_n(RegTarget::E),
        0x26 => cpu.ld_r_n(RegTarget::H),
        0x2E => cpu.ld_r_n(RegTarget::L),
        0x3E => cpu.ld_r_n(RegTarget::A),
...
impl CPU {
    ...
    pub fn ld_r_n(&mut self, target: RegTarget) -> Result<CyclesType> {
        ...
        match target {
            RegTarget::A => self.registers.set_a(value),
            RegTarget::B => self.registers.set_b(value),
            RegTarget::C => self.registers.set_c(value),
            RegTarget::D => self.registers.set_d(value),
            RegTarget::E => self.registers.set_e(value),
            RegTarget::H => self.registers.set_h(value),
            RegTarget::L => self.registers.set_l(value),
            RegTarget::HL => {
                let addr = self.registers.get_hl();
                self.write_byte(addr, value)?;
            }
    ...
    pub fn ld_nn_sp(&mut self) -> Result<CyclesType> {
        ...
        // TODO: 這裡應取得 MMU 實例並寫入記憶體，暫以註解標示
        // self.mmu.write_byte(address, (sp & 0xFF) as u8)?;
        // self.mmu.write_byte(address + 1, (sp >> 8) as u8)?;
...
// LD r,r 指令 (40~7F)
pub fn ld_r_r(cpu: &mut CPU, dest: RegTarget, src: RegTarget) -> CyclesType {
    // TODO: 實作 LD r,r
    4
}

// LD r,n 指令 (06, 0E, 16, 1E, 26, 2E, 36, 3E)
pub fn ld_r_n(cpu: &mut CPU, dest: RegTarget, value: u8) -> CyclesType {
    // TODO: 實作 LD r,n
    8
}

// LD r,(HL) 指令 (4E, 5E, 6E, 7E)
pub fn ld_r_hl(cpu: &mut CPU, dest: RegTarget) -> CyclesType {
    // TODO: 實作 LD r,(HL)
    8
}

// LD (HL),r 指令 (70~75, 77)
pub fn ld_hl_r(cpu: &mut CPU, src: RegTarget) -> CyclesType {
    // TODO: 實作 LD (HL),r
    8
}

// LD (HL),n 指令 (36)
pub fn ld_hl_n(cpu: &mut CPU, value: u8) -> CyclesType {
    // TODO: 實作 LD (HL),n
    12
}

// LD A,(C) 指令 (F2)
pub fn ld_a_c(cpu: &mut CPU) -> CyclesType {
    // TODO: 實作 LD A,(C)
    8
}

// LD (C),A 指令 (E2)
pub fn ld_c_a(cpu: &mut CPU) -> CyclesType {
    // TODO: 實作 LD (C),A
    8
}

// LD A,(nn) 指令 (FA)
pub fn ld_a_nn(cpu: &mut CPU, addr: u16) -> CyclesType {
    // TODO: 實作 LD A,(nn)
    16
}

// LD (nn),A 指令 (EA)
pub fn ld_nn_a(cpu: &mut CPU, addr: u16) -> CyclesType {
    // TODO: 實作 LD (nn),A
    16
}
