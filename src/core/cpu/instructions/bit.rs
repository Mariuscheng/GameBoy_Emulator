use super::prelude::*;
use super::register_utils::{FlagOperations, RegisterOperations};
use crate::core::cycles::{CYCLES_2, CYCLES_3, CYCLES_4, CyclesType};
...
impl CPU {
    pub fn rlc_r(&mut self, target: &RegTarget) -> Result<CyclesType> {
    ...
    pub fn rrc_r(&mut self, target: &RegTarget) -> Result<CyclesType> {
        let value = self.get_register_value(target)?;
    ...
    pub fn bit_b_r(&mut self, bit: u8, reg: &RegTarget) -> Result<CyclesType> {
        ...
        self.increment_pc(2)?; // CB prefix + opcode
...
#[cfg(test)]
mod tests {
    use super::super::register_utils::FlagOperations;
    ...
    fn test_rlc_b() {
        ...
        cpu.registers.set_b(0b1000_0001);
        let _ = super::dispatch(&mut cpu, 0x00); // CB 00 = RLC B
        assert_eq!(cpu.registers.get_b(), 0b0000_0011);
    ...
    fn test_bit_7_b() {
        let mut cpu = test_cpu();
        cpu.registers.set_b(0b1000_0000);
        let _ = super::dispatch(&mut cpu, 0x78); // CB 78 = BIT 7,B
        assert!(!cpu.registers.get_zero());
        cpu.registers.set_b(0b0000_0000);
    ...
    fn test_set_3_c() {
        let mut cpu = test_cpu();
        cpu.registers.set_c(0b0000_0000);
        let _ = super::dispatch(&mut cpu, 0xD9); // CB D9 = SET 3,C
        assert_eq!(cpu.registers.get_c(), 0b0000_1000);
    }

    #[test]
    fn test_res_0_d() {
        let mut cpu = test_cpu();
        cpu.registers.set_d(0b0000_0001);
        let _ = super::dispatch(&mut cpu, 0x82); // CB 82 = RES 0,D
        assert_eq!(cpu.registers.get_d(), 0b0000_0000);
    }
}
