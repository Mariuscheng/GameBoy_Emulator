...
pub fn decode_and_execute(cpu: &mut CPU, mmu: &mut MMU, opcode: u8) {
    ...
    match opcode {
        ...
        0x0C | 0x14 | 0x1C | 0x24 | 0x2C | 0x34 | 0x3C => {
            ...
            let value = registers.get_by_index(reg).wrapping_add(1);
            registers.set_by_index(reg, value);
        }
        // INC DIV（已移至 0x04，避免重複）
        // DEC r
        0x05 | 0x0D | 0x15 | 0x1D | 0x25 | 0x2D | 0x35 | 0x3D => {
            ...
            let value = registers.get_by_index(reg).wrapping_sub(1);
            registers.set_by_index(reg, value);
        }
        // PUSH qq
        0xC5 => {
